...
const NavItem = ({ category, isOpen, handleOpen, isAnyOpen }: NavItemProps) => {
    ...
    const allImageSrc = PRODUCT_CATEGORIES.flatMap(category =>
        category.featured.map(item => item.imageSrc)
    );
            ...
            {isOpen ? (
                <div className={cn("absolute inset-x-0 top-full text-muted-foreground text-sm ",
                    ...
                    <div className="absolute inset-0 top-1/2 bg-white shadow "
                        aria-hidden='true'
                    />
                    <div className="relative bg-white max-h-30 overflow-auto ">
                            ...
                            <div className="grid grid-cols-4 gap-x-8 gap-y-10 py-16 ">
                                ...
                                {category.featured.map((item)=>(
                                        <div key={item.name} className='group relative text-base sm:text-sm'>
                                            ...
                                            <Link href={item.href} className='mt-6 block font-medium text-gra'>
                                                {item.name}
                                                </Link>
